...
-- Ape keys
CREATE TABLE IF NOT EXISTS ape_keys (
  _id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  uid TEXT NOT NULL,
  name TEXT NOT NULL,
  hash TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_results_uid_timestamp ON results(uid, timestamp DESC);
...
CREATE INDEX IF NOT EXISTS idx_connections_key ON connections(key);
CREATE INDEX IF NOT EXISTS idx_presets_uid ON presets(uid);
CREATE INDEX IF NOT EXISTS idx_ape_keys_uid ON ape_keys(uid);
CREATE INDEX IF NOT EXISTS idx_new_quotes_language ON new_quotes(language);
CREATE INDEX IF NOT EXISTS idx_new_quotes_approved ON new_quotes(approved);
CREATE INDEX IF NOT EXISTS idx_logs_uid ON logs(uid);
CREATE INDEX IF NOT EXISTS idx_leaderboard_entries_lookup ON leaderboard_entries(language, mode, mode2, numbers, rank);
