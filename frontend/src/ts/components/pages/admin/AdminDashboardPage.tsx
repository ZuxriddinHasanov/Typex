    ...
    <AdminLayout active="dashboard" title="Dashboard">
      ...
      <div class="mt-6 grid gap-6 lg:grid-cols-2">
        ...
        <div class="rounded-2xl border border-sub/10 bg-bg/60 p-5">
          ...
          >
            <div class="custom-scrollbar max-h-64 space-y-1 overflow-y-auto pr-2">
              <For each={activityQuery.data}>
                {(p: any) => (
                  <div class="flex items-center justify-between rounded-lg bg-sub-alt/30 px-4 py-2 text-xs">
                    <span class="font-medium text-text">{p.date}</span>
                    <span class="font-semibold text-sub">
                      <i class="fas fa-keyboard mr-1 text-main"></i> {p.tests}{" "}
                      test <span class="mx-1">•</span>{" "}
                      <i class="fas fa-users mr-1 text-main"></i> {p.users}
                    </span>
                  </div>
                )}
              </For>
        ...
        <div class="rounded-2xl border border-sub/10 bg-bg/60 p-5">
          <h2 class="mb-4 text-sm font-bold text-text">
            Eng faol foydalanuvchilar (Top 20)
          </h2>
          <Show
                ...
                {(u: any, i) => (
                  ...
                  >
                    <div class="flex items-center gap-3">
                      <div class="w-5 text-right font-black text-main">
                        #{i() + 1}
                      </div>
                      <DiscordAvatar
                        ...
                        fallbackIcon="user"
                      />
                      <span class="font-bold text-text">{u.name}</span>
                    </div>
                    <div class="flex items-center gap-3 font-semibold">
                      <span class="text-sub" title="Yakunlangan testlar soni">
                        <i class="fas fa-check-circle text-green-500 mr-1"></i>
                        {u.tests}
                      </span>
                      <span class="rounded-md bg-main/10 px-2 py-0.5 text-main">
                        {Math.round(u.wpm)} WPM
                      </span>
                    </div>
                  </a>
                )}
