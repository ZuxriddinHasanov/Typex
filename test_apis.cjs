const test = async (name, url_uz, url_en) => {
  try {
    const [rUz, rEn] = await Promise.all([fetch(url_uz), fetch(url_en)]);
    ...
    console.log(`${name}: OK=${ok}, Same=${sameCount}, UzCount=${uzCount}, EnCount=${enCount}`);
    if (!ok) console.log("  ERR:", dUz.message || dUz, dEn.message || dEn);
  } catch (e) {
...
const run = async () => {
  ...
  await test('Time 30 All-Time', 
    `${base}/leaderboards?mode=time&mode2=30&language=uzbek&page=0&pageSize=50`,
    `${base}/leaderboards?mode=time&mode2=30&language=english&page=0&pageSize=50`);
    
  await test('Time 60 All-Time', 
    `${base}/leaderboards?mode=time&mode2=60&language=uzbek&page=0&pageSize=50`,
    `${base}/leaderboards?mode=time&mode2=60&language=english&page=0&pageSize=50`);
  ...
  await test('Weekly XP', 
    `${base}/leaderboards/weekly?language=uzbek&page=0&pageSize=50`,
    `${base}/leaderboards/weekly?language=english&page=0&pageSize=50`);
};
